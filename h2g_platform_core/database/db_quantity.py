...
class QuantityDB(QuantityDAO):
    ...
    QUANTITY = {
        'value': PREFIX.H2G_PROPERTY.alias_uri('propertyValue'),
        'unit': PREFIX.H2G_PROPERTY.alias_uri('hasUnit'),
    }
    ...
    SELECT = """SELECT ?kind ?unit ?value
                WHERE {{{{
                    ...
                    {{uri}} a ?kind;
                           {1} ?value;
                           {2} ?unit.
            }}}}""".format(PREFIX.H2G_PROPERTY.alias_uri('PhenomenonProperty'),
                           QUANTITY["value"],
                           QUANTITY["unit"])
    ...
    def create(self, quantity):
        ...
        """
        _id = str(generate_id())
        _url = PREFIX.H2G_PROPERTY.alias_uri(_id)
        # Build the query
        query = """INSERT DATA {{
                {0} a {1};
                """.format(_url,
                           PREFIX.H2G_PROPERTY.alias_uri(quantity.kind))
    ...
    def create_for(self, quantity, relation, parent_url):  # , url=None):
        ...
        query = 'INSERT DATA {{{} {} {}}}'.format(
            parent_url, relation, PREFIX.H2G_PROPERTY.alias_uri(_id))
        self.onto_mgr.perform(SPARQLOP.INSERT, query)
        return _id
    ...
    def get_all_uris_for(self, url, relation=None, kind=None):
        ...
        if kind is None:
            _kind = PREFIX.H2G_PROPERTY.alias_uri('PhenomenonProperty')
        else:
            _kind = PREFIX.H2G_PROPERTY.alias_uri(kind)
        # Build query
        query = """SELECT ?uri
                   WHERE {{
                       {} {} ?uri.
