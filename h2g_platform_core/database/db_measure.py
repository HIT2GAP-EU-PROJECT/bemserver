...
class MeasureDB(ThingDB):
    ...
    OPT_LINKS = {
        # 'property': PREFIX.H2G_PROPERTY.alias_uri('hasMeasurementProperty'),
    ...
    REFERENCES_ENUM = {
        'medium': {
            ...
            'indiv': False},
        'observation_type': {
            'prefix': PREFIX.H2G_PROPERTY,
            'type': PREFIX.H2G_PROPERTY.alias_uri('PhenomenonProperty'),
            'indiv': False},
        'unit': {
            'prefix': [PREFIX.UNIT, PREFIX.H2G_PROPERTY],
...
#############################################
    ...
    def _build_create_query(self, _id, element):
        ...
        query += "{rel} {pref}:{id};".format(
            ...
            pref=PREFIX.H2G_PROPERTY.alias, id=element.observation_type)
