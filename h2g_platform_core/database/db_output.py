...
class OutputDB(ThingDB):
    ...
    REFERENCES_ENUM = {
        'kind': {'prefix': PREFIX.H2G_PROPERTY,
                 'type': PREFIX.H2G_PROPERTY.alias_uri('Property'),
                 'indiv': False},
        'unit': {'prefix': [PREFIX.UNIT, PREFIX.H2G_PROPERTY],
...
# ######### INSERT #################
    ...
    def _build_create_query(self, _id, element):
        ...
        query.append("{rel} {obj};".format(
            ...
            obj=PREFIX.H2G_PROPERTY.alias_uri(element.values_desc.kind)))
        return "".join(query + ["}"])
