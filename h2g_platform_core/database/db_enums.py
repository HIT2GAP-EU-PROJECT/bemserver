...
class DBEnumHandler(AbsDBEnum):
    ...
    def get_window_covering_types(self):
        """Return window covering types (from h2gBI ontology)."""
        result = self._get_enum('WindowCoveringType', PREFIX.BUILDING_INFRA)
        result.label = 'WindowCovering'
        return result

    def get_hemisphere_types(self):
        """Return hemisphere types (from h2gSensor ontology)."""
        return self._get_enum('Hemisphere', PREFIX.H2G_SENSOR)

    def get_climate_types(self):
        """Return climate types (from h2gProperty ontology)."""
        return self._get_enum('Climate', PREFIX.BUILDING_INFRA)

    def get_orientation_types(self):
        """Return orientation types (from h2gBI ontology)."""
    ...
    def get_occupant_states(self):
        """Return occupant states (from h2gProperty ontology)"""
        result = self._get_enum('OccupantStateProperties', PREFIX.H2G_PROPERTY)
        result.label = 'OccupantState'
        return result

    def get_gender_types(self):
        """Return gender types (from h2g ontology)."""
    ...
    def get_observation_types(self):
        """Get values for observation types."""
        result = self._get_enum('PhysicalProperty', PREFIX.H2G_PROPERTY)
        res_presence = self._get_enum('OccupantProperty', PREFIX.H2G_PROPERTY)
