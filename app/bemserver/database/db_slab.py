...
from .schemas import SlabSchema
from .utils import str_insert, str_filter, str_filter_relation


class SlabDB(StructuralElementDB):
    ...
    LINKS = {
        ...
        "floors": PREFIX.BUILDING_INFRA.alias_uri('inContactWith'),
        "building": PREFIX.BUILDING_INFRA.alias_uri('contains'),
    }

    REFERENCES = {
        "building_id": PREFIX.IFC2x3.alias_uri('IfcBuilding'),
        "floors": [PREFIX.IFC2x3.alias_uri('IfcBuildingStorey')],
    }
    ...
    FILTERS_REF = {
        "floor_id": "?URI {rel} {prefix}:{{id}}\n".format(
            rel=LINKS['floors'], prefix=PREFIX.ROOT.alias)
    }

    CLASS_TO_FIELD_TO_RELATION = {
        Slab: FIELD_TO_RELATION,
    ...
    def _build_select_query(self, identifier=None, **filters):
        ...
        query = """{sel} WHERE {{
                           ...
                           ?cls rdfs:label ?kind.
                """.format(sel=select,
                           clss=PREFIX.IFC2x3.alias_uri('IfcSlab'),)
    ...
    def _pre_load_binding(self, binding):
        ...
        binding['floors'] = list(self.get_related_individuals_id(
            PREFIX.ROOT.alias_uri(binding['id']),
            self.LINKS["floors"],
    ...
    def _build_create_query(self, _id, element):
        _class = PREFIX.IFC2x3.alias_uri('IfcSlab') if not element.kind else \
            PREFIX.BUILDING_INFRA.alias_uri(element.kind)
        # Build the query
        query = """{{
                {0} a {1};
                    {2} "{3}" ;
    ...
    def create(self, element):
        ...
        for elt_id in element.floors:
            self._create_relation_to(my_uri, self.LINKS['floors'],
